...
export const PersonalInfoSection: React.FC<PersonalInfoSectionProps> = props => {
    const generateDescriptionComponent = () => {
        if (!props.user?.selfDescription || props.user?.selfDescription.length === 0)
            return null;
    ...
    const generateCertificationsComponent = () => {
        if (!props.user?.certifications || props.user?.certifications.length === 0)
            return null;
        return (
            props.user?.certifications?.map((certificate) => (
                <Stack key={certificate.name} direction={"row"} spacing={1}>
                    <Box width={50} height={"70"} bgcolor={"silver"} display={"flex"} justifyContent={"center"} alignItems={"center"}>
                    ...
                    <Stack>
                        ...
                        <Stack direction={"row"} color={grey[500]} spacing={1}>
    ...
    return (
        <Stack spacing={4}>
            ...
            <PersonalInfoItem label='Kỹ năng' emptyLabel="Thông tin về kỹ năng của bạn khiến nhà tuyển dụng để mắt đến bạn hơn">
                {generateSkillsComponent()}
            </PersonalInfoItem>
            <PersonalInfoItem label='CHỨNG CHỈ' emptyLabel="Thêm các chứng chỉ mà bạn có giúp gây ấn tưởng với nhà tuyển dụng">
